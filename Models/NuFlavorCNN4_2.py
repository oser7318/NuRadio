...
def conv_block(nfilters, nlayers=1, dropout=False, **kwargs):
    ...
    for _ in range(nlayers):
        model.add(Conv2D(nfilters, config.kernel_size,strides=config.strides, padding="same", activation="relu", **kwargs))
...
#Dense
model.add(Flatten()) #output 40960
...
history = model.fit(x=dataset_train, steps_per_epoch=steps_per_epoch, epochs=30,
