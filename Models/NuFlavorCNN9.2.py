...
hyperparam_default = dict(
    kernel_size = (1,15), #(1,10) originally
    poolsize = (1,5),
    lr = 1e-4,
)
...
conv_block(32, nlayers=2)
conv_block(64, nlayers=3)
conv_block(128, nlayers=4)
conv_block(256, nlayers=4)
...
history = model.fit(x=dataset_train, steps_per_epoch=steps_per_epoch, epochs=50,
          validation_data=dataset_val, callbacks=[checkpoint, csv_logger, WandbCallback(), reduce_lr])
